...
public class HttpUtil {
    ...
    public static String doPost(String url, String json) {
        ...
        try {
            ...
            client.getHttpConnectionManager().getParams().setConnectionTimeout(5000); // 连接5秒超时
            client.getHttpConnectionManager().getParams().setSoTimeout(30000);// 读取30秒超时
    ...
    @SuppressWarnings({ "rawtypes", "unchecked" })
    public static String HttpGet(String url, String method, Map paramMap,String encod) {
    	//设置编码格式
        String encoding = encod;
        String webUrl = url + "/" + method;
        if (encoding == null || "".equals(encoding))
            encoding = encod;
        String queryString = createLinkString(paramMap);
